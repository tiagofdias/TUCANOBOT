const { Events, ChannelType, Collection, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, PermissionFlagsBits, PermissionOverwrites, Permissions, ModalBuilder } = require('discord.js');

let voiceManager = new Collection();
const voiceConnections = new Map();

// Create a function to calculate the time spent in the voice channel and log it
function logTimeSpentInVoiceChannel(userId) {
  const startTime = voiceConnections.get(userId);
  if (!startTime) return; // User is not in a voice channel

  const timeSpent = Date.now() - startTime;
  console.log(`User ${userId} spent ${timeSpent}ms in the voice channel.`);
}
...
module.exports = {
  ...
  async execute(oldState, newState) {
    ...
    if (oldChannel !== newChannel && newChannel && newChannel.id === '882632556372176937') {

      const sourceCategory = newChannel.parent;

      // Get the user and the voice channel they are in
      ...
      return setTimeout(() => {
        ...
        // If the user left the voice channel or isn't in a voice channel, remove their entry from the map
        if (!voiceChannel || oldState.channel === voiceChannel) {
          console.log('Entrou')
          voiceConnections.delete(user.id);
          return;
        }

      /*   // If the user just joined the voice channel, add their entry to the map with the current time
        if (oldState.channel !== voiceChannel) {
          voiceConnections.set(user.id, Date.now());
        }

        // If the user left the voice channel, calculate and log the time spent in the voice channel
        if (oldState.channel && oldState.channel.members.has(user.id) && !voiceChannel.members.has(user.id)) {
          const startTime = voiceConnections.get(user.id);
          const timeSpent = Date.now() - startTime;
          console.log(`User ${user.id} spent ${timeSpent}ms in the voice channel.`);
          voiceConnections.delete(user.id);
        } */
