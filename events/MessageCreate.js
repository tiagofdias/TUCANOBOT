...
module.exports = {
	...
	async execute(message) {
		...
		try {
			...
			if (query)  //UPDATE
			{
				const nextLevelXp = calculateLevelXp(query.level + 1);

				query.xp += xpToGive;

				if (query.xp >= nextLevelXp) {
					query.level++;
					message.member.send(`${message.member} you have leveled up to **level ${query.level}**.`);
				}
